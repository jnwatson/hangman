#!/bin/bash
# Bootstrap CAX41 for k=6 depth=3: install, rsync source, rsync existing k=6 cache, build, run.
set -eu
IP="$1"
SSH_OPTS="-o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/home/nic/.ssh/known_hosts_hangman -o ConnectTimeout=15"
SSH="ssh $SSH_OPTS"
log() { echo "[$(date -Iseconds)] [${IP} k=6 d=3] $*"; }
...
log "adding 16 GB swap (insurance for RAM growth)"
$SSH root@$IP 'fallocate -l 16G /swapfile && chmod 600 /swapfile && mkswap /swapfile > /dev/null && swapon /swapfile && free -h | grep Swap:'
...
log "rsyncing existing k=6 depth=2 cache (~7 GB)"
$SSH root@$IP 'mkdir -p /root/hangman2/game_cache'
rsync -az --stats -e "$SSH" \
    /home/nic/proj/hangman2/game_cache/tt_len6_e5395d4ba5709f78 \
...
HOST=\$(hostname)
curl -sd "START \$HOST k=6 depth=3 (cax41 nbg1)" "https://ntfy.sh/\$NTFY" > /dev/null
cd /root/hangman2
/usr/bin/time -v ./target/release/precompute --dict enable1.txt --lengths 6 --cache-dir game_cache --depth 3 --max-words 999999 > /root/precompute.log 2>&1
RC=\$?
curl -sd "END \$HOST k=6 depth=3 rc=\$RC" "https://ntfy.sh/\$NTFY" > /dev/null
RUN
